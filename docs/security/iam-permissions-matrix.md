...
| Serving SPA (CloudFront → S3) | `s3:GetObject` | Oggetti del bucket frontend statico | Servire gli asset della SPA Angular | LocalStack / futura prod | In LocalStack il bucket è **public-read** (`Principal: "*"`) solo per l'emulazione del CloudFront locale. In prod: bucket **privato** con CloudFront **OAC** (principal `cloudfront.amazonaws.com`, condizione `AWS:SourceArn` sulla distribuzione), **mai** `Principal: "*"`. |
