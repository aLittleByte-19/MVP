...
Questo documento mette in relazione i controlli applicativi della PoC con le aree dell'OWASP
Application Security Verification Standard. È una **baseline PoC** con **allineamento parziale**:
...
| Area | Baseline implementata nella PoC | Evidenza nella codebase | Rischio residuo | Lavoro production-like |
| --- | --- | --- | --- | --- |
...
| Sicurezza upload | Gli upload PDF sono validati (contenuto + estensione, dimensione, pagine) e archiviati su object storage con path generati server-side; preview via stream con autorizzazione per tenant, senza URL firmati. | `app/Http/Requests/UploadDocumentRequest.php`, `app/Http/Controllers/` (preview) | Nessuna scansione antivirus né CDR: un PDF malevolo viene comunque inoltrato a Bedrock e ri-servito in preview. | Finalizzazione presigned S3 con validazione metadata server-side e hook antivirus/CDR. |
